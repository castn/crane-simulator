...
def TrussAnalysis(): 
  ...
  Kff = K[np.ix_(freeD0F,freeD0F)]
  Kfr = K[np.ix_(freeD0F,supportD0F)]
  Krf = Kfr.T Krr = K[np.ix_(supportD0F,supportD0F)]
  Pf = P.flatten()[freeD0F] Uf = np.linalg.solve(Kff,Pf)
...
def Plot(nodes,c,lt,lw,lg):
  ...
  for i in range(len(bars)):
    xi, xf = nodes[bars[i3O],0], nodes[bars[i,1],0]
    yi, yf = nodes[bars[i3O],1], nodes[bars[i,1],1]
    zi, zf = nodes[bars[i3O],2], nodes[bars[i,1],2]
    line, = plt.plot([xi, xf],[yi, yf],[zi, zf],color=c,linestyle=lt,linewidth=lw)
  line.set_label(lg)
  plt.legend(prop=f'size': 8})
