...
class PlotterManager:
    ...
    def update_optimized_plots(self, nodes, opt_deformed_nodes, beams, opt_area_per_rod, optm_axial_forces,
        ...
        simple_plot(nodes, beams, "grey", "--", "Undeformed", self.optim_ax_l, end_crane_parts[1], end_crane_parts[2])
        plot_deformation_with_grad(opt_deformed_nodes, beams, '-', self.optim_ax_l, optm_axial_forces,
    ...
    def update_diff_plot(self, base_nodes, base_opt_deformed_nodes, base_beams, base_optm_axial_forces, current_nodes,
                         current_opt_deformed_nodes, current_beams, current_optm_axial_forces, base_end_crane_parts,
                         current_end_crane_parts):
        # Comparison base
        if not type(base_nodes) == type(None) or not type(base_opt_deformed_nodes) == type(None) or not type(
            ...
            simple_plot(base_nodes, base_beams, "grey", "--", "Undeformed", self.diff_ax_l, base_end_crane_parts[1],
                        base_end_crane_parts[2])
        ...
        simple_plot(current_nodes, current_beams, "grey", "--", "Undeformed", self.diff_ax_r,
                    current_end_crane_parts[1], current_end_crane_parts[2])
